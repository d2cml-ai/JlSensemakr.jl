function ovb_bounds(model, treatment; benchmark_covariates = nothing, kd = 1, ky = nothing, alpha = 0.5, 
    h0 = 0, reduce = true, bound = "partial_r2", adjusted_estimates = true)
...
function ovb_partial_r2_bound(; model = nothing, treatment::Union{Nothing, String} = nothing, r2dxj_x = nothing, 
    ...
    if !isnothing(model)
        m = DataFrame(model.mm.m, columns = coefnames(ols))
        d = m[:, treatment]
        non_treatment = Matrix([:, Not(treatment)])
        non_treatment = hcat(ones(size(non_treatment, 1)), non_treatment)
        treatment_fit = lm(non_treatment, d)

        r2dxj_x = Vector(undef, length(benchmark_covariates))
        r2yxj_dx = Vector(undef, length(benchmark_covariates))
        if benchmark_covariates isa Array
            for b in benchmark_covariates
                r2dxj_x[b] = group_partial_r2(treatment_fit, b)
                r2yxj_dx[b] = group_partial_r2(model, b)
            end
        elseif benchmark_covariates isa Dict
            for b in keys(benchmark_covariates)
                r2dxj_x[b] = group_partial_r2(treatment_fit, benchmark_covariates[b])
                r2yxj_dx[b] = group_partial_r2(model, benchmark_covariates[b])
            end
        end
    ...
    for i in eachindex(benchmark_covariates)
        ...
        r2zxj_xd = kd * (r2dxj_x[i] ^ 2) ./ ((1 .- kd * r2dxj_x[i]) * r2dxj_[i])
        ...
        if kd isa Real
            ...
            for j in eachindex kd
                bound_label = label_maker(benchmark_covariates[i], kd = kd[j], ky = ky[j])
